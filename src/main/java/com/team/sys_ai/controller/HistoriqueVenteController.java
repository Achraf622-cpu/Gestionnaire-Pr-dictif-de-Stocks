...
public class HistoriqueVenteController {
        ...
        public ResponseEntity<PageResponse<HistoriqueVenteDTO>> getHistoriqueByEntrepot(
                ...
                return ResponseEntity.ok(PageResponse.from(
                                historiqueVenteService.getHistoriqueByEntrepot(entrepotId, userDetails.getUser(),
                                                pageable)));
        }
        ...
        public ResponseEntity<PageResponse<HistoriqueVenteDTO>> getHistoriqueByDateRange(
                        @PathVariable Long entrepotId,
                        ...
                        @Parameter(hidden = true) @AuthenticationPrincipal CustomUserDetails userDetails) {
                return ResponseEntity.ok(PageResponse.from(
                                historiqueVenteService.getHistoriqueByEntrepotAndDateRange(
                                                entrepotId, startDate, endDate, userDetails.getUser(), pageable)));
        }
        ...
        public ResponseEntity<PageResponse<HistoriqueVenteDTO>> getHistoriqueByProduit(
                        @PathVariable Long entrepotId,
                        @PathVariable Long produitId,
                        @PageableDefault(size = 20, sort = "dateVente", direction = Sort.Direction.DESC) Pageable pageable,
                        @Parameter(hidden = true) @AuthenticationPrincipal CustomUserDetails userDetails) {
                return ResponseEntity.ok(PageResponse.from(
                                historiqueVenteService.getHistoriqueByProduitAndEntrepot(
                                                produitId, entrepotId, userDetails.getUser(), pageable)));
        }
        ...
        public ResponseEntity<HistoriqueVenteDTO> recordSale(
                ...
                if (dateVente != null) {
                        ...
                        result = historiqueVenteService.recordSale(entrepotId, produitId, quantite,
                                        userDetails.getUser());
                }
